...
Focus on **narrative framework** and **visual design** for presentations. For technical implementation, refer to `using-typst` skill.
...
├── Invoke researching-deeply skill for topic investigation
...
└── Generate Typst code (technical details: using-typst skill)
      ↓
...
**creating-slides skill 自身需要委托的能力**（这些是 creating-slides skill 没有的能力，需要其他 agent 补充）：
...
| Style Consistency | `@frontend-ui-ux-engineer` | generating-images 生成多张图时保持风格统一 |
...
详细的委托引导、Prompt 模板、术语验证清单见 `reference/frontend-delegation.md`。
...
当使用 `generating-images` skill 生成多张图片时，generating-images 本身是 one-shot 工作，无法保持风格统一。此时需要 creating-slides skill 自己管理风格一致性：
...
**字体是信息层级的骨架。** 遵循 Robert Bringhurst (*The Elements of Typographic Style*) 和 Matthew Butterick (*Practical Typography*) 原则。
...
**颜色传达语义和情绪。** 遵循 Josef Albers (*Interaction of Color*)、WCAG 可访问性标准。
...
Typst 代码示例见 `reference/components.md`。
...
详细检查流程见 `using-typst` skill 的 `reference/touying.md` → "Page Validation" 章节。
...
```plain
[researching-deeply skill] → Gather new information from web
         ↓
[researching-widely skill] → Process existing materials in batch
         ↓
[creating-slides skill] → Narrative design + content transformation
         ↓
[using-typst skill] → Technical implementation
         ↓
[generating-images skill] → AI-generated illustrations (OPTIONAL)
```
...
2. **If missing or empty**: Invoke research skills first
   - `researching-deeply` → for gathering new information from web
   - `researching-widely` → for processing existing local materials
...
概念插图、封面图等**可选**使用 `generating-images` skill。大多数技术分享不需要 AI 生成图片。
...
详细参数和 workflow 见 `generating-images` skill。
